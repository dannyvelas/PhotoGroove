...
var $author$project$PhotoGroove$sizeToClass = function (size) {
	switch (size.$) {
		case 'Small':
			return $elm$html$Html$Attributes$class('small');
		case 'Medium':
			return $elm$html$Html$Attributes$class('med');
		default:
			return $elm$html$Html$Attributes$class('large');
	}
};
	...
	function (photos, selectedUrl, chosenSize) {
		return _List_fromArray(
			[
				...
				_List_fromArray(
					[
						$elm$html$Html$text('Thumbail Size:')
					])),
				A2(
				$elm$html$Html$div,
				_List_fromArray(
